...
{
    public class tbRenewController : ApiController
    {
        #region 用户表

        #region 声明变量
        Maticsoft.BLL.tbRenew BLL = new Maticsoft.BLL.tbRenew();
        ...
        [HttpGet]
        [HttpPost]
        public HttpResponseMessage Save(Maticsoft.Model.tbRenew info)
        {
            HttpResponseMessage result;
            ...
            try
            {
                if (BLL.Exists(info.No,info.id))
                {
                    Maticsoft.Model.tbRenew info1 = (Maticsoft.Model.tbRenew)BLL.GetModel(info.id);
