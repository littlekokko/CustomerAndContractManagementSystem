...
{
    public class tbContractController : ApiController
    {
        #region 用户表

        #region 声明变量
        Maticsoft.BLL.tbContract BLL = new Maticsoft.BLL.tbContract();
        ...
        [HttpGet]
        [HttpPost]
        public HttpResponseMessage Save(Maticsoft.Model.tbContract info)
        {
            HttpResponseMessage result;
            ...
            try
            {
                if (BLL.Exists(info.ContractNo,info.id))
                {
                    Maticsoft.Model.tbContract info1 = (Maticsoft.Model.tbContract)BLL.GetModel(info.id);
