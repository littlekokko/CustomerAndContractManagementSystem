...
{
    public class tbStaffController : ApiController
    {
        #region 用户表

        #region 声明变量
        Maticsoft.BLL.tbStaff BLL = new Maticsoft.BLL.tbStaff();
        ...
        public HttpResponseMessage GetList()
        {
            ...
            try
            {
                DataSet ds = BLL.GetList(1000, "  is_del = 1 ", "BPId");
        ...
        public HttpResponseMessage GetListFenYe(int rows, int page)
        {
            ...
            try
            {
                ...
                DataSet list = BLL.GetListByPage("is_del=1", "BPId", rownumber, (rownumber + rows) - 1);
        ...
        /// <returns></returns>
        [HttpGet]
        public HttpResponseMessage GetDetails(int BPId)
        {
        ...
        [HttpGet]
        [HttpPost]
        public HttpResponseMessage Save(Maticsoft.Model.tbStaff info)
        {
            HttpResponseMessage result;
            ...
            try
            {
                if (BLL.Exists(info.BPId))
                {
                    Maticsoft.Model.tbStaff info1 = (Maticsoft.Model.tbStaff)BLL.GetModel(info.BPId);
        ...
        /// <returns></returns>
        [HttpGet]
        public HttpResponseMessage Delete(int BPId)
        {
            HttpResponseMessage result;
            dynamic o = new { success = true };
        ...
        /// <returns></returns>
        [HttpGet]
        public HttpResponseMessage GetBPId(int BPId)
        {
            HttpResponseMessage result;
            string json;
            string str = " is_del=1 and BPId=" + BPId.ToString() + " ";
            try
            {
                DataSet ds = BLL.GetList(1, str, "BPId");
                dynamic o = new { success = true, rows = ds };
                json = JsonConvert.SerializeObject(o);
            }
            catch (Exception ex)
